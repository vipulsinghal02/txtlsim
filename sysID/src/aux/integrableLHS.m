function int_minit = integrableLHS(eMO, nW, spread, logp,...
    enames, ds, varargin)
...
if iscell(eMO)
    ...
    lhsamp = spread*(lhsdesign(npts, nparam)-0.5);
    ...
    for kk = 1:nOpt
        ...
        count = 1;
        for ii = 1:length(exportedMdlObj.ValueInfo)
            index1 = strcmp(en, exportedMdlObj.ValueInfo(ii).Name);
            index2 = find(index1);
            if index2~= 0
                en2{count} = en{index2};
                count = count+1;
    ...
    nparam = length(logp);
    npts = round(nW*2); % can tolerate up to 50% non integrability. Increase the factor here if you need to tolerate more.
    % generate integrable points
    lhsamp = spread*(lhsdesign(npts, nparam)-0.5);
    minit=bsxfun(@plus,logp,lhsamp');
    tic
