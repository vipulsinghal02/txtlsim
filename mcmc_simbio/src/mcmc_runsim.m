function [tstamp, m] = mcmc_runsim(tstamp, projdir, tv,da, em, mi)
...
loglike = @(logp) sum(lognormvec(gen_residuals_3(logp, em, da, tv,   mi.dosedvals, mi.measuredspecies),sigg));

% run the burn in simulation
 
        tic
        [m] =gwmcmc_vse(minit,{logprior loglike},mi.npoints, 'StepSize',mi.stepsize , ...
    ...
    specificproj = [projdir '/simdata_' tstamp];
        for i = 1:mi.niter %
            ...
            disp(sprintf('starting mcmc %d\n', i));
             [m, lP, nim, rejProp] = gwmcmc_vse(minit,{logprior loglike},mi.npoints, 'StepSize',mi.stepsize , ...
            'ThinChain',mi.thinning, 'Parallel', mi.parallel);
            ...
            cfname{i} = fname;
            save([specificproj '/' fname], 'm', 'lP', 'nim', 'rejProp');
...
[nn] = length(eno);
for i = 1:nn
    fprintf(fileID,fS,eno{i}, exp(mi.paramranges(i, 1)), exp(mi.paramranges(i, 2)));
end
