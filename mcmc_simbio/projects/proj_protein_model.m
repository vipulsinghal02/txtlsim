%% MCMC toolbox demo - proj_protein_model.m
% 
% In this project we demonstrate the use of the toolbox for estimation of 
% parameters for a circuit in two different extracts, with the circuit specific
% parameeters (CSPs) shared between the models and the extract specific 
% parmaeters (ESPs) estimated individually for each extract. 
% 
% We use a model to generate artificial data in two different extracts for 
% known parameters, (different ESPs and the same CSPs). 
%  
% ~~~ MODEL ~~~
% D + pol <-> D__pol  (k_f (fixed!), k_r (Proxy for K_d) ) CSP
% D__pol -> D + pol + protien (kc) ESP
% pol is a ESP too. 
% D is the dosed species
% 
% 
%
% Vipul Singhal, 
...
[da_extract1] = data_artificial(em, mi,tv, 'replicates', round(1), 'params', [100; 20; 10]);

[da_extract2] = data_artificial(em, mi,tv, 'replicates', round(1), 'params', [100; 40; 20]);
