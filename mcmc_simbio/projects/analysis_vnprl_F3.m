...
tstamp = {ts1 ts2 ts3 ts4 ts5 ts6 ts7 ts8 ts9 ts10 ts12 ts13}; % 
nIterID = {1:10 1:10 1:6 1:40 1:30 1:30 1:40 1:40 1:13 1:12 1:9 1:10};% 
load([projdir '/simdata_' ts13 '/full_variable_set_' ts13 '.mat'], ...
    'mi',...
...
%% just the troublesome parameter ranges. 
 close all
mcmc_plot(marray(:, 1:5:end,(end-440):50:end), parnames(:),...
...
%%
mcmc_plot(marray(:, 1:10:end,(end-200):end), parnames(:),...
    'savematlabfig', figsave, 'savejpeg', jpgsave,...
    'projdir', projdir, 'tstamp', tsToSave, ...
    'extrafignamestring','small_hypercube_initialization');
%%
 close all
mcmc_plot(marray(:, :,(end-90):3:end), parnames(:),...
    'savematlabfig', figsave, 'savejpeg', jpgsave,...
    'plotChains', false,...
    'projdir', projdir, 'tstamp', tsToSave, ...
...
%%
 close all
mcmc_plot(marray(selected_params, :,(end-190):50:end), parnames(selected_params),...
    'savematlabfig', figsave, 'savejpeg', jpgsave,...
    'plotChains', false,'fontsize', 22,...
    'projdir', projdir, 'tstamp', tsToSave, ...
