...
%  '20180403_130223'  
%  and the run this is continued from is '20180402_160013'
...
% FULL VERSION
% 0. Make figs A B E F (can be found in note Note Apr 1, 2018 April logbook, 
% April 3 entry). in particular, pick n arbitrary points in the ESP
...
for plotID = 1:size(pToPlot2, 1)
    ...
    CaptureFigVid(...
        ...
        'extract2_animation_stage1',OptionZ)
...
for i = 1:length(paramid2)
    ...
    plot(ax_d10, di(2).timeVector, da(:,1,i,1)+randn(length(di(2).timeVector), 1)*50, 'k','LineStyle', ':', 'LineWidth', 0.02);
    axis(ax_d10,[0 7200 -500 10000])
    hold on
    plot(ax_d30, di(2).timeVector, da(:,1,i,2)+randn(length(di(2).timeVector), 1)*100, 'k','LineStyle', ':', 'LineWidth', 0.02);
    axis(ax_d30,[0 7200 -500 10000])
    hold on
    
    plot(ax_d60, di(2).timeVector, da(:,1,i,3)+randn(length(di(2).timeVector), 1)*200, 'k','LineStyle', ':', 'LineWidth', 0.02);
...
for plotID = 1:size(pToPlot1, 1)
    ...
    CaptureFigVid(...
        veiw_angle_list,...
        'extract1_animation_stage1',OptionZ)
...
for i = 1:length(paramid2)
    ...
    plot(ax_d10, di(1).timeVector, da(:,1,i,1)+randn(length(di(1).timeVector), 1)*50, 'k','LineStyle', ':', 'LineWidth', 0.02);
    axis(ax_d10, [0 7200 -500 10000])
    hold on
    plot(ax_d30, di(1).timeVector, da(:,1,i,2)+randn(length(di(1).timeVector), 1)*100, 'k','LineStyle',':', 'LineWidth', 0.02);
    axis(ax_d30,[0 7200 -500 10000])
    hold on,
    
    pp = plot(ax_d60, di(1).timeVector, da(:,1,i,3)+randn(length(di(1).timeVector), 1)*200, 'k','LineStyle',':', 'LineWidth', 0.02);
    axis(ax_d60, [0 7200 -500 10000])
    hold on
