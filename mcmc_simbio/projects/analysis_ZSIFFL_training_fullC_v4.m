...
projdir = [pwd '\mcmc_simbio\projects\proj_ZSIFFL_trainingC_v4'];
addpath(projdir)
...
switch tsIDtouse
    case 1 % this is after including the termination parameters.
        ...
        tstamp = {ts17 ...
             ts18 ts19 ts20 ts21 ts22 ts23 ts24 ts25 ts26 ts27};
        nIterID = {1:20 1:7 1:20 1:5  1:20  1:20  1:20  1:20 1:20 1:20 1:20};
...
mcmc_plot(marray(:, 1:20:end,1:end), parnames(:),...
    ...
    'projdir', projdir, 'tstamp', tsToSave);
%%
mcmc_plot(marray(:, 1:end,end-600:40:end), parnames(:),...
    'savematlabfig', figsave, 'savejpeg', jpgsave,...
